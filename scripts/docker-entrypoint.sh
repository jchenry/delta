...
cust="${GEN_DIR}/modules/go-server-lambda-openapi-generator"
codegen="${cli}/target/openapi-generator-cli.jar"
custgen="${cust}/target/go-server-lambda-openapi-generator-0.0.0-SNAPSHOT.jar"
...
if [[ -f "${codegen}" && -n "$(java ${JAVA_OPTS} -jar "${codegen}" completion | grep "^$1\$" )" ]]; then
    command=$1
    shift
    exec java ${JAVA_OPTS} -cp "${codegen}:${custgen}" org.openapitools.codegen.OpenAPIGenerator "${command}" "$@"
